...
vec3 brightnessToColor(float brightness){
    brightness *= 0.25;
    return (vec3(brightness, brightness*brightness, brightness*brightness*brightness*brightness) / 0.25) * 0.8;
}
...
void main() {
    float sun = supersun();
    float brightness = sun * 4.0 + 0.75;
    float fres = Fresnel(eyeVector,vNormal);
    float updatedBrightness = brightness + fres;
    vec3 col = brightnessToColor(updatedBrightness);
