...
export default class Sun {
  ...
  addGlowEffect(mesh) {
    const spriteMaterial = new THREE.SpriteMaterial({ 
      map: new THREE.TextureLoader().load('/textures/sun_surface.jpg'), 
      color: 0xffdd99, 
      transparent: true,
      opacity: 0.7,
    ...
    glowSprite.scale.set(this._threeRadius * 3, this._threeRadius * 3, 1); 
...
vec3 sunColor(float intensity) {
    
    return mix(vec3(1.0, 0.8, 0.2), vec3(1.0, 0.4, 0.1), intensity);
}

vec3 glowColor(float glowFactor) {
  
    return mix(vec3(1.0, 0.8, 0.2), vec3(1.0, 0.5, 0.0), glowFactor); 
}


void main() {
    float modTime = mod(time, 1000.0); 
    vec4 p = vec4(vPosition, modTime);
    float noisy = fbm(p);
    float normalizedNoise = (noisy + 1.0) * 0.5; 
    vec3 baseColor = sunColor(normalizedNoise);

    vec3 finalColor = mix(baseColor, baseColor, 1.0);

    float distanceFromCenter = length(gl_PointCoord - vec2(0.5)); 
    float glowIntensity = smoothstep(0.2, 0.5, distanceFromCenter); 

    vec3 glow = glowColor(glowIntensity);

    
    finalColor = mix(finalColor, glow, glowIntensity * 0.8); 
